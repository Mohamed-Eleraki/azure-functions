        ...
        with open(csv_path, "r", newline='', encoding="utf-8") as csvfile:
            ...
            for row in reader:
                paths_list.append({
                    ...
                    "src_sas_token": row["src_sas_token"],
                })
...
def csv_src_path_handler(paths_list, blob_names):
    ...
    Replaces $BLOB_NAME in source paths with a blob name and appends SAS tokens.
    ...
    Args:
        ...
        blob_names (list[str]): A list of blob names to replace the wildcard in the source paths.

    Returns:
        list[str]: A list of updated source paths with $BLOB_NAME replaced and SAS tokens appended.
    """
    updated_src_paths = []
...
def csv_dest_path_handler(paths_list, blob_names):
    ...
    Replaces $BLOB_NAME in destination paths with a blob name and appends SAS tokens.
    ...
    Args:
        ...
        blob_name blob_names (list[str]): A list of blob names to replace the wildcard in the source paths.

    Returns:
        list[str]: A list of updated destination paths with $BLOB_NAME replaced and SAS 
        tokens appended.
    """
